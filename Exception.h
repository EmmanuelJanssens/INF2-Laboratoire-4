...
{
public:
	explicit vecteur_index_error(const std::string& s) : logic_error("VECTEUR : index error " + s) { }
...
{
public:
	explicit vecteur_length_error(const std::string& s) : logic_error("VECTEUR : length error " + s) { }
	explicit vecteur_length_error(const char* c) : logic_error(c) {}
};
class  vector_length_mismatch : public std::logic_error
{
public:
	explicit vector_length_mismatch(const std::string& s) : logic_error("VECTEUR : length mismatch " + s) { }
...
{
public:
	explicit not_square_matrix(const std::string& s) : logic_error("MATRIX : illegal operation on non square matrix " +s) { }
...
{
public:
	explicit matrix_size_mismatch(const std::string& s) : logic_error("MATRIX : matrix size mismatch "+s) { }
...
{
public:
	explicit vecteur_range_error(const std::string& s) : runtime_error("VECTEUR : could not resize vecteur " + s) { }
